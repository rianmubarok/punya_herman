<x-app-layout>
    <div class="bg-gradient-to-br from-blue-50 to-purple-100 min-h-screen py-6 flex items-center justify-center">
        <div class="max-w-xl w-full bg-gradient-to-br from-white via-blue-50 to-purple-100 p-8 rounded-2xl shadow-lg border border-blue-200 animate-fade-in">
            ...
            <div class="mb-2 text-gray-700"><strong>User:</strong> {{ $portfolio->user->name ?? '-' }} <span class="text-xs text-gray-400">({{ $portfolio->user->email ?? '' }})</span></div>
            ...
            <div class="mb-2 text-gray-700"><strong>Skill:</strong> 
                @foreach(explode(',', $portfolio->skills) as $skill)
                    @if(trim($skill) !== '')
                        <span class="inline-block bg-purple-100 text-purple-700 text-xs px-2 py-1 rounded-full font-semibold mr-1">{{ trim($skill) }}</span>
                    @endif
                @endforeach
            </div>
            @if($portfolio->image)
            ...
            <a href="{{ route('admin.portfolios.index') }}" class="inline-block mt-4 px-6 py-2 bg-gradient-to-r from-blue-500 to-purple-500 text-white font-semibold rounded shadow hover:from-blue-600 hover:to-purple-600 transition-all duration-200">Kembali ke daftar</a>
        </div>
    </div>
    <style>
        @keyframes fade-in {
