    ...
    <div class="bg-gradient-to-br from-blue-50 to-purple-100 min-h-screen py-6 flex items-center justify-center">
        <div class="max-w-xl w-full bg-white p-8 rounded-2xl shadow-lg border border-blue-200 animate-fade-in">
            <div class="flex flex-col items-center mb-4">
                <img src="https://undraw.co/api/illustrations/edit_file.svg" alt="Edit Portfolio Illustration" class="w-20 h-20 mb-2" loading="lazy">
                <h1 class="text-2xl font-bold text-blue-900 text-center">Edit Portofolio</h1>
            </div>
            <form action="{{ route('admin.portfolios.update', $portfolio) }}" method="POST" enctype="multipart/form-data" class="space-y-5 animate-fade-in">
                @csrf @method('PUT')
                <div>
                    ...
                    <select name="user_id" required class="w-full border border-purple-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-300 focus:border-blue-400 transition-all duration-200">
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" @if($user->id == old('user_id', $portfolio->user_id)) selected @endif>{{ $user->name }} ({{ $user->email }})</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $portfolio->title) }}" required class="w-full border border-purple-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-300 focus:border-blue-400 transition-all duration-200">
                ...
                <div>
                    ...
                    <textarea name="description" required class="w-full border border-purple-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-300 focus:border-blue-400 transition-all duration-200">{{ old('description', $portfolio->description) }}</textarea>
                ...
                <div>
                    ...
                    <input type="text" name="skills" value="{{ old('skills', $portfolio->skills) }}" class="w-full border border-purple-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-300 focus:border-blue-400 transition-all duration-200">
                ...
                <div>
                    ...
                    @if($portfolio->image)
                        <img src="{{ asset('storage/'.$portfolio->image) }}" width="100" class="mb-2 rounded shadow">
                    @endif
                ...
                <div>
                    ...
                    <input type="url" name="project_link" value="{{ old('project_link', $portfolio->project_link) }}" class="w-full border border-purple-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-300 focus:border-blue-400 transition-all duration-200">
                    @error('project_link') <div class="text-red-500 text-sm mt-1">{{ $message }}</div> @enderror
                </div>
                <div class="flex gap-3 mt-6">
                    <button type="submit" class="px-6 py-2 bg-gradient-to-r from-blue-500 to-purple-500 text-white font-semibold rounded-lg shadow hover:from-blue-600 hover:to-purple-600 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 transition-all duration-200 transform hover:scale-105">Update</button>
